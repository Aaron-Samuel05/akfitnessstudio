...
import { ArrowDown, ArrowUpRight, Check, Clock3, Instagram, MapPin, Menu, Phone, X } from 'lucide-react';
...
const plans = [
  { name: 'STARTER', copy: 'A simple entry point for building a consistent training habit.', items: ['Full gym access', 'Equipment orientation', 'Goal consultation'] },
  { name: 'PERFORMANCE', copy: 'Structured coaching for people who want measurable progress.', items: ['Full gym access', 'Training programme', 'Progress tracking', 'Coach support'], featured: true },
  { name: 'PERSONAL', copy: 'One-to-one attention with a plan built around your body and goals.', items: ['Personal training', 'Individual programme', 'Progress assessments', 'Nutrition guidance'] },
];
const classes = [['06:00','STRENGTH','MON / WED / FRI','60 MIN'],['07:30','CONDITIONING','MON / TUE / THU','45 MIN'],['18:00','PERFORMANCE','MON / WED / FRI','60 MIN'],['19:15','FUNCTIONAL','TUE / THU / SAT','50 MIN']];
const faqs = [['Do I need to be experienced?','No. We coach beginners and experienced lifters. Your starting point determines the programme, not your ego.'],['Can I try the gym before joining?','Yes. Use the Join Now form and choose Free Trial. The team can confirm the best available slot.'],['Do you offer personal training?','Yes. Personal training can be built around strength, body composition, performance and your individual goals.'],['What should I bring?','Training clothes, shoes, water and the willingness to work. We handle the rest.']];

export default function Home() {
  const [menu, setMenu] = useState(false); const [joinOpen, setJoinOpen] = useState(false); const [faq, setFaq] = useState<number | null>(null);
  ...
  return <main>
    ...
    <header className="nav video-site-nav">
      ...
      <nav className="nav-links"><button onClick={() => go('training')}>TRAINING</button><button onClick={() => go('programs')}>PROGRAMS</button><button onClick={() => go('space')}>THE SPACE</button><button onClick={() => go('membership')}>MEMBERSHIP</button><button onClick={() => go('contact')}>CONTACT</button></nav>
      <button className="nav-cta" onClick={() => setJoinOpen(true)}>JOIN NOW <ArrowUpRight size={15}/></button><button className="menu-btn" onClick={() => setMenu(!menu)} aria-label="Open navigation">{menu ? <X size={22}/> : <Menu size={22}/>}</button>
    </header>
    ...
    <section className="statement" id="training"><div className="section-index">01 / THE METHOD</div><div><p className="eyebrow">NOT A MEMBERSHIP. A STANDARD.</p><h2>TRAIN WITH<br/><span>INTENTION.</span></h2><p className="statement-copy">AK is built around the idea that great training should be coached, measurable and repeatable. Whether you are starting from zero or chasing your next level, every session has a purpose.</p><button className="lime-link" onClick={() => go('programs')}>EXPLORE TRAINING <ArrowDown size={16}/></button></div><div className="statement-side">01<br/><span>COACHED<br/>WORK</span></div></section>

    <section className="image-break"><SafeImage src={images.athlete} alt="Muscular man training in a modern gym"/><div className="image-break-caption"><span>REAL PHYSIQUE</span><span>REAL WORK</span><span>NO SHORTCUTS</span></div></section>

    <section className="programs" id="programs"><div className="section-index">02 / PROGRAMS</div><div className="section-heading"><p className="eyebrow">FIND YOUR LANE</p><h2>BUILT<br/><span>AROUND YOU.</span></h2></div><div className="program-grid">{[['01','STRENGTH','Build force, muscle and confidence.','strength'],['02','CONDITIONING','Move better. Work harder. Recover faster.','training'],['03','PERSONAL TRAINING','A programme built around your body and your goals.','athlete']].map(([num,title,copy,img]) => <article className="program-card" key={num}><SafeImage src={images[img as keyof typeof images]} alt={`${title} training at AK Fitness`}/><div className="program-overlay"/><span className="program-num">{num}</span><div className="program-bottom"><h3>{title}</h3><p>{copy}</p><ArrowUpRight/></div></article>)}</div></section>

    <section className="space" id="space"><div className="space-copy"><p className="eyebrow">03 / THE SPACE</p><h2>THE FLOOR<br/><span>MATTERS.</span></h2><p>Serious equipment. Open training space. A floor designed for movement, strength and people who actually want to train.</p><button className="lime-link" onClick={() => go('gallery')}>SEE THE FLOOR <ArrowUpRight size={16}/></button></div><div className="space-image"><SafeImage src={images.floor} alt="Gym floor and equipment"/><span>01 / THE FLOOR</span></div></section>

    <section className="gallery" id="gallery"><div className="gallery-head"><div className="section-index">04 / THE FLOOR</div><h2>COME<br/><span>SEE IT.</span></h2><p>Don&apos;t judge a gym from a brochure. Walk onto the floor.</p></div><div className="gallery-grid"><SafeImage className="gallery-tall" src={images.strength} alt="Strength training"/><SafeImage src={images.training} alt="Fitness training"/><SafeImage src={images.athlete} alt="Athlete workout"/></div></section>
